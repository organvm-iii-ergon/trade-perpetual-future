    ...
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold flex items-center gap-2">
          <DiceThree size={28} weight="duotone" className="text-amber" />
          PvP Games
        </h2>
        <p className="text-foreground/70 mt-1">Challenge players in provably fair games</p>
      </div>
      ...
      <div>
        ...
        {activeGames.length === 0 ? (
          <div className="glass-strong rounded-3xl p-12 text-center">
            ...
            <p className="text-foreground/70">No open games</p>
            <p className="text-sm text-foreground/60 mt-1">Create a game to get started</p>
          </div>
        ) : (
          <div className="grid md:grid-cols-2 gap-4">
            {activeGames.filter(g => g.status === 'waiting').map((game) => (
              <div key={game.id} className="glass-strong rounded-2xl p-4 border border-white/10">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <Badge variant="secondary" className="gap-1 glass">
                      {game.type === 'dice' && <DiceThree size={14} weight="duotone" />}
                      {game.type === 'coinflip' && <CoinVertical size={14} weight="duotone" />}
                      {game.type === 'price-prediction' && <ChartLineUp size={14} weight="duotone" />}
                  ...
                  <Badge variant="outline" className="glass">Waiting</Badge>
                </div>
                <div className="space-y-2 mb-4">
                  <div className="flex justify-between text-sm">
                    <span className="text-foreground/70">Wager</span>
                    <span className="font-mono font-semibold">${game.wager.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-foreground/70">Potential Win</span>
                    <span className="font-mono font-semibold text-long">
    ...
    <div className="space-y-4">
      <div>
        ...
        <Input
          ...
          step="0.01"
          min="1"
          className="font-mono mt-2 glass h-12"
        />
      ...
      <div className="glass rounded-2xl p-4 space-y-2 text-sm border border-white/10">
        ...
        <div className="flex justify-between">
          <span className="text-foreground/70">Fee ({(GAME_FEE_PERCENT * 100).toFixed(0)}%)</span>
          <span className="font-mono font-semibold">${(parseFloat(wager || '0') * 2 * GAME_FEE_PERCENT).toFixed(2)}</span>
        </div>
        <div className="flex justify-between font-semibold">
          <span>Potential Win</span>
          <span className="font-mono text-long text-lg">${potentialWin.toFixed(2)}</span>
        </div>
      </div>
