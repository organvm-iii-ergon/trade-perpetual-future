...
`trade-perpetual-future` is a TS+Python-based project within ORGAN-III (Ergon) of the organvm eight-organ creative-institutional system. The project needed a technology foundation that balances rapid prototyping with long-term maintainability, aligning with the organ system's emphasis on portfolio-quality engineering.

## Decision

We chose TS+Python as the primary implementation language, leveraging its ecosystem for scientific computing, data processing, and AI/ML integration. The project follows the organvm repository standards (documented in `docs/standards/10-repository-standards.md` of the planning corpus) and integrates with the cross-organ dependency graph maintained in `registry-v2.json`.

Key architectural choices:
- **Language**: TS+Python — selected for ecosystem fit and team expertise
